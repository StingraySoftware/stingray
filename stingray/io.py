...
try:
    ...
    _FITSIO_INSTALLED = True
except ImportError:
    _FITSIO_INSTALLED = False
...
class FITSTimeseriesReader(object):
    ...
    def __init__(
        self, fname, output_object_kind=None, gti_file=None, gtistring=None, additional_columns=None
    ):
        self.fname = fname
        self._meta_attrs = []
        self.gtistring = gtistring
        self.output_object_kind = output_object_kind
        self.additional_columns = additional_columns
        self._initialize_header(fname)
    ...
    def __getitem__(self, index):
        new_ts = self.output_object_kind()
    ...
    def fitsio_filter(self, filter_expression, fitsio_hdu=None):
        ...
        This function should be faster than astropy.io.fits to access very large
        datasets with complex filtering expressions.
        ``fitsio`` is efficient at filtering large datasets without the need to load
        the entire dataset in memory, or creating masks of the same dimension as the
        dataset.

        ``fitsio`` has to be installed to use this function.

        Parameters
        ----------
        filter_expression : str
            A string containing the filtering expression. It has to be compatible with
            the fitsio filtering syntax. For example, to filter events between times 10 and 20
            use ``"TIME >= 10 && TIME < 20"``.

        Other Parameters
        ----------------
        fitsio_hdu : FITS HDU object, optional
            The HDU object to filter. If not provided, the HDU object of ``self.fname`` in
            the HDU name specified in ``self.hduname`` will be used.

        Returns
        -------
        filter_spec: array-like
            An array of indices of the events that satisfy the filtering expression.

        """
        if not _FITSIO_INSTALLED:
            raise RuntimeError("fitsio_filter requires fitsio")

        if fitsio_hdu is None:
            fitsio_hdu = fitsio.FITS(self.fname)[self.hduname]
